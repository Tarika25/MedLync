...
export async function sendSignupOTP(phone: string) {
  const res = await fetch(`${API_BASE}/auth/send-signup-otp`, { method: "POST", headers: getHeaders(), body: JSON.stringify({ phone }) });
  const json = await res.json();
  if (!res.ok) throw new Error(json.error);
  return json;
}

export async function verifySignupOTP(phone: string, otp_code: string) {
  const res = await fetch(`${API_BASE}/auth/verify-signup-otp`, { method: "POST", headers: getHeaders(), body: JSON.stringify({ phone, otp_code }) });
  const json = await res.json();
  if (!res.ok) throw new Error(json.error);
  return json;
}
