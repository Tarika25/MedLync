...
app.post('/api/auth/send-signup-otp', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO signup_otps (phone, otp_code, created_at) VALUES ($1, $2, NOW())
       ON CONFLICT (phone) DO UPDATE SET otp_code = $2, created_at = NOW(), is_verified = FALSE`,
      [phone, otp_code]
    );
    await twilioClient.messages.create({
      body: `Your MedLync signup OTP is: ${otp_code}. Valid for 10 minutes.`,
...
app.post('/api/auth/verify-signup-otp', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT * FROM signup_otps WHERE phone = $1 AND otp_code = $2 AND is_verified = FALSE
       AND created_at > NOW() - INTERVAL '10 minutes'`,
      [phone, otp_code]
    );
    if (rows.length === 0) return res.status(400).json({ error: 'Invalid or expired OTP' });
    await pool.query('UPDATE signup_otps SET is_verified = TRUE WHERE phone = $1', [phone]);
...
app.post('/api/auth/signup', async (req, res) => {
  ...
  try {
    ...
    const { rows: otpRows } = await pool.query(
      'SELECT 1 FROM signup_otps WHERE phone = $1 AND is_verified = TRUE',
      [phone]
    );
    if (otpRows.length === 0) return res.status(400).json({ error: 'Phone number not verified' });
...
app.get('/api/prescriptions/verify', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT p.*, 
        json_build_object('name', u.name, 'phone', u.phone, 'profile_photo_url', u.profile_photo_url) AS patient,
        (SELECT json_agg(json_build_object('id', m.id, 'name', m.name, 'dosage', m.dosage, 'frequency', m.frequency, 'duration', m.duration, 'refill_count', m.refill_count, 'end_date', m.end_date)) FROM medicines m WHERE m.prescription_id = p.id) AS medicines
       FROM prescriptions p 
       LEFT JOIN users u ON p.patient_id = u.id 
...
// Send OTP
app.post('/api/prescriptions/send-otp', authenticate, async (req, res) => {
  const { prescription_id, phone_number } = req.body;
  try {
    const otp_code = generateOTP();
    const otp_id = uuidv4();

    await pool.query(
      `INSERT INTO otp_verifications (id, prescription_id, phone_number, otp_code)
       VALUES ($1, $2, $3, $4)`,
      [otp_id, prescription_id, phone_number, otp_code]
    );

    await twilioClient.messages.create({
      body: `Your MedLync OTP for prescription pickup is: ${otp_code}. Valid for one-time use only.`,
      from: process.env.TWILIO_PHONE_NUMBER,
      to: phone_number,
    });

    res.json({ otp_id, message: 'OTP sent successfully' });
  } catch (err) {
...
// Verify OTP
app.post('/api/prescriptions/verify-otp', authenticate, async (req, res) => {
  const { otp_id, otp_code } = req.body;
  try {
    const { rows } = await pool.query(
      'SELECT * FROM otp_verifications WHERE id = $1 AND otp_code = $2 AND is_verified = FALSE',
      [otp_id, otp_code]
    );

    if (rows.length === 0) return res.status(400).json({ error: 'Invalid OTP' });

    await pool.query('UPDATE otp_verifications SET is_verified = TRUE WHERE id = $1', [otp_id]);
...
app.post('/api/pharmacy/dispense', authenticate, async (req, res) => {
  const { prescription_code, collected_by, otp_id } = req.body;
  try {
    // Verify OTP if family pickup
    if (collected_by === 'family' && otp_id) {
      const { rows: otpRows } = await pool.query(
        'SELECT * FROM otp_verifications WHERE id = $1 AND is_verified = TRUE',
        [otp_id]
      );
      if (otpRows.length === 0) return res.status(400).json({ error: 'OTP not verified' });
    }

    const { rows } = await pool.query(
      'SELECT id FROM prescriptions WHERE prescription_code = $1',
      [prescription_code]
    );
    if (rows.length === 0) return res.status(404).json({ error: 'Prescription not found' });

    const prescription_id = rows[0].id;
